...
class ExperimentNikolaComb(Experiment):
    """
    ...
    def create_loss_fn(self):
        ...
        def asymmetric_loss(pred, label):
            return loss_gaze_net(pred, label) + loss_e(pred, label)
        ...
        def loss_gaze_net(pred, label):
            ...
            e_l = angular_error(pred["gaze-left"], gaze_left, mean=False)
            e_r = angular_error(pred["gaze-right"], gaze_right, mean=False)
            e_combined = angular_error(pred['final-pred'], gaze_right, mean=False)

            l_asymmetric = 2.0*(e_l*e_r)/(e_l+e_r+1e-6)

            l_symmetric = (e_l+e_r)/2.0

            l_combined = e_combined
            ...
            return torch.mean(w*l_asymmetric + (1.0-w)*beta*l_symmetric + gamma*l_combined)
            # TODO Option 1 -  torch.mean(w*l_asymmetric + (1.0-w)*l_combined)
            # TODO Option 2 - beta and gamma .... play arround with them
