...
class HDF5Dataset(Dataset):
    ...
    def __init__(self, hdf_path, transform_l, transform_r, use_colour = True, data_format = 'NCHW', preprocessing = False):
        """
        Args:
        ...
        if 'test' in os.path.basename(hdf_path):
            self.landmarks = ['eye-region', 'face', 'face-landmarks', 'head', 'left-eye', 'right-eye']
        else:
    ...
    def __getitem__(self, idx):        
        with h5py.File(self.hdf_path, 'r',libver='latest',swmr=True) as dataset_file:
            ...
            if self.transform_l:
                sample["left-eye"] = self.transform_l(Image.fromarray(np.uint8(sample["left-eye"])))
            if self.transform_r:
                sample["right-eye"] = self.transform_r(Image.fromarray(np.uint8(sample["right-eye"])))
